...
class HeroDownloader(object):
    ...
    def parseHeroLinks(self):
        stream = urllib.urlopen(self.baseLink)
    ...
    def createHeroDatabase(self):
        newDatabase = open('HeroDatabase.txt', 'w')
        for heroName in self.getHeroNames():
            print(heroName, file=newDatabase)
        newDatabase.close()
...
print('Writing heroes to database ...')
downloader.createHeroDatabase()
print('Done writing heroes to database')
